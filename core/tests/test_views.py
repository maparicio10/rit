...
class ViolationReportAPITest(APITestCase):
    def setUp(self):
        self.person = Person.objects.create(first_name="John", last_name="Doe", email="john.doe@example.com")
        self.officer = Officer.objects.create_user(username="officer1", password="testpassword", first_name="Jane",
                                                   last_name="Doe", email="jane.doe@example.com")
        self.officer.identification_number = uuid.uuid4().int % 10 ** 10
        self.officer.save()
        self.vehicle = Vehicle.objects.create(license_plate="ABC123", brand="Toyota", color="Red", person=self.person)
    ...
    def test_generar_informe(self):
        ...
        response = self.client.post(url, {'email': 'john.doe@example.com'}, format='json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_generar_informe_missing_email(self):
        ...
        self.assertEqual(response.data['error'], "El correo electrónico es obligatorio.")

    def test_generar_informe_invalid_email(self):
        url = reverse('generar_informe')
        response = self.client.post(url, {'email': 'invalid-email'}, format='json')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['error'], "El correo electrónico proporcionado no es válido.")
